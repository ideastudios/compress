...
def welcome():
    ...
    if temp_type == 1:
        print('')
        print('4) 请输入需要压缩的 excel 文件路径(绝对路径):')
        print('    也可以将 excel 文件拖到这个窗口')
        input_path = input("完成后按 '回车' 确认：")

        print('')
        print('5) 请输入压缩后的 excel 文件保存目录(绝对路径):')
        ...
        print("excel 文件中图片压缩成功，excel 保存路径为:")
        print(compress_path)
        open_folder(output_path)
        print("快去看看吧")
    elif temp_type == 2:
        print('')
        print('4) 请输入需要压缩图片夹文件夹路径(绝对路径):')
        print('    也可以将文件拖到这个窗口')
        input_path = input("完成后按 '回车' 确认：")

        print('')
        print('5) 请输入压缩后的图片文件保存路径(绝对路径):')
        print('    也可以将需要存放的文件夹拖到这个窗口')
        output_path = input("    完成后按 '回车' 确认：")

        print('')
        print('6) 请确认是否将 png 转为 jpg格式，更节省空间:')
        ...
        import photo
        photo.compressImage(input_path, output_path, False,
